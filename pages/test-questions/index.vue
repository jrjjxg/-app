  ...
  <view class="flex flex-col h-full bg-gray-50">
    ...
    <view v-else class="flex-1 px-4 py-6">
      <view class="bg-white rounded-xl p-5 shadow-sm mb-6">
        ...
        <text v-if="currentQuestion.description" class="text-sm text-gray-500 mb-4 block">{{ currentQuestion.description
          }}</text>
      </view>

      <!-- 选项 -->
      <view class="space-y-3">
        <view v-for="(option, index) in currentQuestion.options" :key="index" class="bg-white rounded-xl p-4 shadow-sm"
          :class="{ 'border-2 border-blue-500 bg-blue-50': selectedOption === index }" @click="selectOption(index)">
    ...
    <view class="p-4 bg-white border-t border-gray-200">
      <view class="flex justify-between">
        <button class="px-6 py-3 rounded-full border border-gray-300 text-gray-600 font-medium"
          :disabled="currentQuestionIndex === 0" :class="{ 'opacity-50': currentQuestionIndex === 0 }"
          @click="prevQuestion">
          上一题
        </button>

        <button class="px-6 py-3 rounded-full bg-blue-600 text-white font-medium" :disabled="selectedOption === null"
          :class="{ 'opacity-50': selectedOption === null }" @click="nextQuestion">
